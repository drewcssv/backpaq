...
BLYNK_WRITE(V29) {  // V29 is the Virtual Pin assigned to the Button Widget
    if (param.asInt()) {  // User desires private data mode, no write to Thingspeak
...
BLYNK_WRITE(V15) {
 ...
 void calcAQIValue() {
   // Set value, color of AQI gauge on Blynk ////////
...
/********* Send To Thinkspeak  Channel  891066  ***************/
void SendToThingspeak(void) // Send to Thingspeak using Webhook
{ 
 if (privateMode == 0) { // check for data privacy..."0" = send to Thingspeak, "1" = NOT send
...
// for (int i=0; i<savedPos; i++) {
  ...
  //  Blynk.virtualWrite(V16, i+1, latISav[i], lonISav[i], i+100); // update GPS position from iphone sensor on map     
 // }  
}
